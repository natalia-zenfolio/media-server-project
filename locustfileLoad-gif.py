# Usage: locust -f locustfileLoadz.py --no-web -c 1 -r 1 -t 15m --only-summary --csv=output
...
class MyTaskSet(TaskSet):
    ...
    def on_start(self):
        ...
        global photoId
        photoId = self.upload(config.image_file_gif, config.image_format_gif)
    ...
    def upload(self, image_file_gif, image_format_gif):
        auth = 'Bearer ' + config.token
        filename = os.path.basename(config.image_file_gif)
        encoder = MultipartEncoder(
            fields={'galleryTypeCode': 'family', 'photoTypeCode': 'booking-photo',
                    'photoFile': (filename, open(image_file_gif, 'rb'), image_format_gif)})
        header = {
            'Authorization': auth,
            'Ocp-Apim-Subscription-Key': config.apim_key,
    ...
    def query_image(self):
        ...
        with self.client.get(url=config.query_url + '/' + photoId, params=param, headers=header, name=config.query_url) as response:
            ...
            file_size = os.path.getsize(config.image_file_gif)
            assert (file_size == downloaded_size), "Got %d, expected %d" % (file_size, downloaded_size)
            assert (downloaded_format == config.image_format_gif), "Got %s, expected %s" % (downloaded_format, config.image_format_gif)
