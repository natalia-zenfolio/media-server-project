# Usage: locust -f locustfileLoad-gif.py --no-web -c 1 -r 1 -t 15m --only-summary --csv=output
...
class MyTaskSet(TaskSet):
    ...
    def query_image(self):
        ...
        with self.client.get(url=config.query_url + '/' + photoId, params=param, headers=header, name=config.query_url) as response:
            ...
            assert (downloaded_format == config.image_format), "Got %s, expected %s" % (downloaded_format, config.image_format)
