...
class MyTaskSet(TaskSet):
    ...
    def on_start(self):
        ...
        global photoId
        photoId = self.upload(config.image_file_heic, config.image_format_heic)
    ...
    def upload(self, image_file_heic, image_format_heic):
        auth = 'Bearer ' + config.token
        filename = os.path.basename(config.image_file_heic)
        encoder = MultipartEncoder(
            fields={'galleryTypeCode': 'family', 'photoTypeCode': 'booking-photo',
                    'photoFile': (filename, open(image_file_heic, 'rb'), image_format_heic)})
        header = {
            'Authorization': auth,
            'Ocp-Apim-Subscription-Key': config.apim_key,
    ...
    def query_image(self):
        ...
        with self.client.get(url=config.query_url + '/' + photoId, params=param, headers=header, name=config.query_url) as response:
            ...
            file_size = os.path.getsize(config.image_file_heic)
            assert (file_size == downloaded_size), "Got %d, expected %d" % (file_size, downloaded_size)
            assert (downloaded_format == config.image_format_heic), "Got %s, expected %s" % (downloaded_format, config.image_format_heic)
